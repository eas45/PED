...
// Elimina los elementos de las posiciones pares
//  (se entiende que respecto a las posiciones originales)
void
TABBCom::FiltrarLista (TListaCom& lista) const
{
  // Se recupera la segunda posición
  TListaPos siguientePos = lista.Primera().Siguiente();
  TListaPos posicionBorrar;

  // Elimina los nodos pares
  while (!siguientePos.EsVacia())
  { // Mientras la siguiente posición par no esté vacía, se seguirá eliminando
    posicionBorrar = siguientePos;
    // Se consigue la siguiente posicion a borrar
    siguientePos = posicionBorrar.Siguiente().Siguiente();
    // Se borra la posición par
    lista.Borrar(posicionBorrar);
  }
}

// SIEMPRE BUSCARÁ UN NODO QUE EXISTE
int
TABBCom::NivelNodo (const TComplejo& complejo) const
{
  if (!EsVacio())
  {
    if (complejo == Raiz())
    ...
    if (complejo < Raiz())
    {
      if (!nodo->iz.EsVacio())
      {
        return 1 + nodo->iz.NivelNodo(complejo);
      }
      return 0;
    }
    else
    { // Comprueba si puede moverse hacia el hijo derecho
      if (!nodo->de.EsVacio())
      {
        return 1 + nodo->de.NivelNodo(complejo);
      }
      return 0;
    }
  }
...
TComplejo*
TABBCom::MostrarNiveles (TListaCom& lista)
{
  TComplejo* arrayDinamico = NULL;

  if (!lista.EsVacia())
  {
    // Recorrer lista de izda a dcha y eliminar posiciones pares
    if (lista.Longitud() > 1)
    { // Si hay más de 1 elemento en la lista, entonces se filtrará
      FiltrarLista(lista);
      cout << "Lista -> " << lista << endl;
    }

    // Se debe copiar los elementos de la lista en un array dinámico
    arrayDinamico = new TComplejo[lista.Longitud()];
    TListaPos posicion = lista.Primera();
    // Se copian los elementos de la lista de izda a dcha
    for (int i = 0; i < lista.Longitud(); i++)
    {
      arrayDinamico[i] = posicion.Pos()->Elemento();
      posicion = posicion.Siguiente();
    }

    // Se elimina el árbol si tiene elementos
    if (!EsVacio())
    {
      this->~TABBCom();
    }
    ...
    for (int i = 0; i < lista.Longitud(); i++)
    {
      Insertar(arrayDinamico[i]);
    }

    // Muestra las inserciones
    TVectorCom niveles = Niveles();
    //cout << "Niveles " << niveles << endl;
    for (int i = 0; i < lista.Longitud(); i++)
    {
      TComplejo elemento = arrayDinamico[i];
      int nivel = NivelNodo(elemento);
      cout << "{Elemento: " << elemento << ". Elementos de su nivel:";
      for (int j = 0; j < lista.Longitud(); j++)
      {
        //cout << "Nodo : " << niveles[j + 1] << endl;
        if (NivelNodo(niveles[j+1]) == nivel)
        {
          cout << " " << niveles[j+1];
        }
      }
